...
public class SessionService2 {
	...
	public boolean bookSession(Subscriber sub, Session session) {
		...
		if (session.getConnection() == null) {
			...
			if (connection.equals("G4")) {
				achievedDataRate = (int) (300 * session.getDataRate());
			} else if (connection.equals("G3")) {
			...
			while (usedSec <= session.getSeconds()) {
				...
				usedSec += 1;
				usedVolume += achievedDataRate;
				//update total used data volume of subscriber
				sub.setDataVolume(sub.getDataVolume() + achievedDataRate);
			}

			print += "Achieved data-rate: " + achievedDataRate + " Mbit/s\n";
			print += "Used data volume: " + usedVolume + "\n";
